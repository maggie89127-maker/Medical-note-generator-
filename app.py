...
SYSTEM_PROMPT_BASE = r"""You are an experienced clinical physician and medical educator.
...
CRITICAL: The user may provide TWO blocks of information:
1. Main clinical data (ER note, OPD note, SP script)
2. Supplemental data (marked with "--- 補充資料 ---")

You MUST incorporate BOTH into the note. For any content that originates from the supplemental data block, wrap it with [[...]] double brackets in the Present Illness section ONLY. Example: [[The patient also reported intermittent headache for 2 weeks.]]
This marking is critical for the display system. Only mark supplemental content in Present Illness.
...
過去病史 (Past History)
List: Chronic diseases, Operation history, Previous admission history
...
Before writing the ROS, determine the patient's sex from the clinical data.
- For MALE patients: Do NOT include female-specific items (e.g. menstruation, pregnancy, vaginal discharge).
- For FEMALE patients: Do NOT include male-specific items (e.g. prostate symptoms). Add gynecological items if clinically indicated.
...
9. MANDATORY: [[double brackets]] around supplemental-data content in Present Illness only.
...
PI_FULL = """Narrative paragraph style. Include:
- Patient info (age, sex, ADL)
- Underlying diseases
...
If ER course mentioned: Vital signs as compact line: T:XX.X P:XX R:XX SBP:XXX DBP:XX E:X V:X M:X SPO2:XX%
Lab abnormals, imaging findings.
ER meds: name only, no dosages.
Conclude: Under the impression of ..., admitted for further management.
Remember: wrap any content from supplemental data with [[...]]."""

PI_SHORT = """CONCISE surgical-style present illness. 4-8 sentences max.
Key elements only: age/sex, chief complaint, onset/duration, key positives/negatives, vital signs (compact line), critical lab/imaging, ER interventions (drug names only, no doses).

MANDATORY LINE BREAK RULES:
1. ER/hospital arrival description → insert ONE BLANK LINE before it.
2. "Under the impression of..." → insert ONE BLANK LINE before it.

End with: Under the impression of ..., admitted for further management.
No redundancy. Direct and efficient.
Remember: wrap any content from supplemental data with [[...]]."""
...
def build_system_prompt(surgical=False):
...
🔵 補充資料在PI中以<span style="color:#2471a3;font-weight:700;">藍字</span>標示。🛡️ 自動去識別化。</div>""",
    ...
    if gen_med or gen_surg:
        ...
        if not active_key:
            ...
            sys_prompt = build_system_prompt(surgical=surgical)
